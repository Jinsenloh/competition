...
def wait_snapshot(
    ...
    consultation_payload = get_consultation_payload(consultation_id)
    message_payload = list_messages_payload(consultation_id)
    consultation = consultation_payload["consultation"]
    messages = message_payload["messages"]
    latest_message_id = messages[-1]["id"] if messages else None
    latest_updated_at = consultation.get("updated_at")

    if after_message_id is None:
        has_new_messages = False
    else:
        message_ids = [message["id"] for message in messages]
        has_new_messages = after_message_id not in message_ids or latest_message_id != after_message_id

    status_changed = bool(after_updated_at and latest_updated_at != after_updated_at)
    return {
...
async def wait_consultation_payload(
    consultation_id: str,
    after_message_id: str | None = None,
    after_updated_at: str | None = None,
    timeout_seconds: int = WAIT_TIMEOUT_SECONDS,
) -> dict[str, Any]:
    timeout_seconds = max(0, min(timeout_seconds, WAIT_TIMEOUT_SECONDS))
    initial = wait_snapshot(consultation_id, after_message_id, after_updated_at)
    if after_message_id is None:
        after_message_id = initial["latest_message_id"]
    if after_updated_at is None:
        after_updated_at = initial["latest_updated_at"]
    if initial["consultation"]["status"] == "resolved" or initial["has_new_messages"] or initial["status_changed"]:
        initial.pop("latest_updated_at", None)
        return initial

    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        await asyncio.sleep(min(WAIT_POLL_INTERVAL_SECONDS, max(0, deadline - time.monotonic())))
        snapshot = wait_snapshot(consultation_id, after_message_id, after_updated_at)
        if snapshot["consultation"]["status"] == "resolved" or snapshot["has_new_messages"] or snapshot["status_changed"]:
            snapshot.pop("latest_updated_at", None)
            return snapshot
...
def wait_consultation_payload_sync(
    consultation_id: str,
    after_message_id: str | None = None,
    after_updated_at: str | None = None,
    timeout_seconds: int = WAIT_TIMEOUT_SECONDS,
    ...
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        time.sleep(min(WAIT_POLL_INTERVAL_SECONDS, max(0, deadline - time.monotonic())))
        snapshot = wait_snapshot(consultation_id, after_message_id, after_updated_at)
        if snapshot["consultation"]["status"] == "resolved" or snapshot["has_new_messages"] or snapshot["status_changed"]:
            snapshot.pop("latest_updated_at", None)
            return snapshot
...
@support_mcp.tool(
    name="wait_for_consultation_update",
    description="Wait up to 5 minutes for a consultation status change, admin reply, resolution, or timeout.",
)
def mcp_wait_for_consultation_update(
    consultation_id: str,
    after_message_id: str | None = None,
    after_updated_at: str | None = None,
    timeout_seconds: int = WAIT_TIMEOUT_SECONDS,
) -> dict[str, Any]:
    return wait_consultation_payload_sync(
        consultation_id,
    ...
    return {
        ...
        "mcp": {
            ...
            "tools": [
                ...
                {
                    "name": "wait_for_consultation_update",
                    "description": "Wait up to 5 minutes for an admin reply, status change, resolution, or timeout.",
                },
                {
                    "name": "get_agent_door_guide",
        ...
        "workflow": [
            ...
            {
                "step": 3,
                "name": "Wait for admin update",
                "method": "GET",
                "path": "/v1/consultations/{id}/wait",
                "result": "Waits up to 300 seconds and returns when messages/status change or timed_out=true.",
            },
            {
                "step": 4,
                "name": "Send updates",
        ...
        "public_endpoints": [
            ...
            },
            {
                "method": "GET",
                "path": "/v1/consultations/{id}/wait",
                "operation_id": "waitForConsultationUpdate",
                "auth": "none",
            },
            {
                "method": "GET",
        ...
        "code_examples": {
            ...
            "wait_for_update": (
                f"curl '{base_url}/v1/consultations/{{consultation_id}}/wait"
                "?timeout_seconds=300&after_message_id={latest_message_id}'"
            ),
            "post_message": (
        ...
        "agent_instructions": [
            ...
            "If the user asks you to wait, call wait_for_consultation_update with timeout_seconds=300 and the latest message id.",
            "If wait_for_consultation_update returns timed_out=true, report the queue number and ask the user to request another check later.",
...
def get_llms_txt(request: Request) -> str:
    ...
    return "\n".join(
        [
            ...
            "4. If the user asks you to wait, call wait_for_consultation_update or GET /v1/consultations/{id}/wait with timeout_seconds=300.",
            "5. If timed_out=true, report the queue number and ask the user to request another check later.",
            "6. POST /v1/consultations/{id}/messages to add user or agent updates.",
            "7. POST /v1/consultations/{id}/handoff when a human support admin is needed.",
            "",
            ...
            '  -d \'{"content":"The user confirmed their account email.","role":"agent","language":"en"}\'',
            "",
            "Wait for admin update example:",
            f"curl '{base_url}/v1/consultations/{{consultation_id}}/wait?timeout_seconds=300&after_message_id={{latest_message_id}}'",
            "",
...
@app.get(
    "/v1/consultations/{consultation_id}/wait",
    tags=["Public Consultations"],
    summary="Wait up to 5 minutes for consultation updates",
    description=(
        "Long-poll endpoint for external AI tools. It waits until a new message, status/update change, "
        "resolution, or timeout. The timeout defaults to 300 seconds and is capped at 300 seconds."
    ),
    operation_id="waitForConsultationUpdate",
)
async def wait_for_consultation_update(
    consultation_id: str,
    after_message_id: str | None = None,
    after_updated_at: str | None = None,
    timeout_seconds: int = WAIT_TIMEOUT_SECONDS,
) -> dict[str, Any]:
    return await wait_consultation_payload(
        consultation_id,
        after_message_id=after_message_id,
        after_updated_at=after_updated_at,
        timeout_seconds=timeout_seconds,
